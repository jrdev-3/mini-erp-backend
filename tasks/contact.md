...
*   [ ] Definir a struct `Contact` em `model.go` contendo as propriedades:
...
*   [ ] Definir a interface `Repository` em `model.go`:
...
*   [ ] Implementar o repositório em `repository.go` em SQL Puro utilizando `pgx/v5`.
    *   [ ] **Segurança contra BOLA:** Todas as queries de escrita e leitura devem aplicar obrigatoriamente a cláusula `WHERE tenant_id = $x` no banco.
    *   [ ] **Mitigação de Enumeração:** Buscar um contato inexistente ou que pertença a outro tenant deve retornar `pgx.ErrNoRows` e responder como `404 Not Found`.

### 3. Camada de Aplicação (`Application`)
*   [ ] Criar as structs de DTO em `dto.go` contendo validações HTTP:
...
*   [ ] Implementar a lógica de negócios em `service.go`:
    *   [ ] Validação de documento único (CPF/CNPJ) dentro de um mesmo tenant.
    *   [ ] Tratamento para aceitar o mesmo documento cadastrado em tenants diferentes (isolamento BOLA corporativo).
